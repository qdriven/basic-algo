...
import io.allroundtesters.adt.tree.TreeStructure;

public class BinarySearchTree<K extends Comparable<K>, V> implements TreeStructure<K, V>,
                                                                     SearchTree<K, V> {
  ...
  private void checkConstruct(BinarySearchTreeNode<K, V> node) {
    ...
    //check
    if (node.getRightChild() != null
        && (node.getRightChild().getEntry().getKey().compareTo(node.getEntry().getKey()) <= 0
            || node
  ...
  /**
   * @param key TODO: 在二叉排序树删去一个结点，分三种情况讨论： 　　若*p结点为叶子结点，即PL(左子树)和PR(右子树)均为空树。由于删去叶子结点不破坏整棵树的结构，
   *            则只需修改其双亲结点的指针即可 。若*p结点只有左子树PL或右子树PR，此时只要令PL或PR直接成为其双亲结点*f的左子树（当 *p是左子树）或右子树（当*p是右子树
   *            ）即可，作此修改也不破坏二叉排序树的特性。若*p结点的左子树和右子树均不空。在删去*p之后 ，为保持其它元素之间的相对位置不变
   *            ，可按中序遍历保持有序进行调整，可以有两种做法：其一是令*p的左子树为*f的左子树，*s为 *f左子树的最右下的结点，而*p的右子树为
   *            *s的右子树；其二是令*p的直接前驱（或直接后继）替代*p，然后再从二叉排序树中删去它的直接前驱（或直接后继）。 不难理解但是过程繁琐，暂时不实现这个
   */
  @Override
